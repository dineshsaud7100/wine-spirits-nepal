...
export default function AdminBrandDetailPage({ params }: { params: Promise<{ slug: string }> }) {
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h1 className="text-2xl font-bold text-white">{brand ? `Edit: ${brand.name}` : "Brand Not Found"}</h1>
              </div>
              ...
              {brand && (
                <form onSubmit={(e) => { e.preventDefault(); router.push("/admin/products"); }} className="space-y-6 max-w-2xl">
                  <div className="bg-luxury-dark/50 border border-white/5 rounded-2xl p-6">
                    <h3 className="text-white font-semibold mb-4">Brand Details</h3>
                    <div className="space-y-4">
                      <div><label className="text-white/60 text-sm mb-1.5 block">Name</label><input defaultValue={brand.name} className={inputClass} /></div>
                      <div><label className="text-white/60 text-sm mb-1.5 block">Country</label><input defaultValue={brand.country} className={inputClass} /></div>
                      <div><label className="text-white/60 text-sm mb-1.5 block">Description</label><textarea defaultValue={brand.description} className={inputClass + " min-h-[80px]"} /></div>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    ...
                    <button type="button" className="px-6 py-2.5 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl font-medium text-sm hover:bg-red-500/20 transition-all flex items-center gap-2"><Trash2 className="w-4 h-4" /> Delete</button>
                  </div>
                </form>
              )}
            </div>
